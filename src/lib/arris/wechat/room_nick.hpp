...
namespace arris {
namespace wechat {
class room_nick :public iwxbase {
public:
	room_nick() {
...
private:
	std::string send_to(const wx_msg& v_st_msg) {
		...
		wchar_t* roomid = wstring_to_wchar(v_st_msg.content);
		...
		std::string s_roomid= ucs2_to_utf8(wstring_to_wchar(v_st_msg.content));
	...
	std::wstring asm_nick(wchar_t* wxid, wchar_t* roomid)
	{
		...
		DWORD dw_call1 = (DWORD)hMod + 0x506C20;//2.8.0.121 =474c30
		DWORD dw_call2 = (DWORD)hMod + 0x36EA10;//2f8430
		DWORD dw_call3 = (DWORD)hMod + 0x50D650;//47b700  50D650
		DWORD dw_call4 = (DWORD)hMod + 0x31AFE0;//2ada70 31AFE0
